...
var formatter = require('./lib/jsonformatter').JSONFormatter;
...
function buildProfileResource(req, profile) {
...
function sendProfile(req, res, next) {
    var profile = buildProfileResource(req, req.profile);
...
function sendProfilesResults(req, res, next) {
    ...
    if (profiles.length > 0) {
        var links = profiles.map(function (profile) {
            return {
                title: profile.name,
                href: getProfileUrl(req, profile.id)
            };
    ...
    res.result = {
        ...
        _embedded: {
            profiles: profiles.map(buildProfileLinks)
        }
    };
...
function addNeedsLinks(req, links, needs) {
    return needs.reduce(function (links, need) {
        var id = need._source.profiles;
        var needType = need._source.type;
...
function sendNeedsResults(req, res, next) {
    ...
    addNeedsLinks(req, needsLinks, needs);
...
function buildNeedLinks(req, need) {
    ...
    var links = {
        profile: {
            href: getProfileUrl(req, need.profiles)
        },
        matching: {
            href: './matching'
        },
...
function buildNeedResource(req, need) {
    need = need._source;

    var links = buildNeedLinks(req, need);
...
function handleNeedResults(next) {
    return function (err, result) {
        if (err) { return next(err); }

        var needs = result.hits.hits.sort(stringSortByField('type'));
...
function sendNeed(req, res, next) {
    ...
    var needResource = buildNeedResource(req, need);
...
function fetchAllNeedsBy(query, next) {
    client.search({
        ...
        body: {
            query: {
                bool: {
                    must: Object.keys(query).map(function (prop) {
                        var q = {}, qType = {};
...
function fetchMatchingNeeds(req, res, next) {
    ...
    var needType = getOtherNeedType(need._source.type);
...
function fetchAllProfilesBy(query, next) {
    var shouldQuery = Object.keys(query).reduce(function (shoulds, prop) {
        var value = query[prop];
...
app.use(function (req, res, next) {
    ...
    res.send(new formatter().jsonToHTML(res.result, null, req.uri));
});
